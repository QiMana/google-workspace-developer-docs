---
source: https://developers.google.com/apps-script/reference/spreadsheet/embedded-column-chart-builder
root: apps-script
fetched_at: 2026-04-23T15:21:37.227Z
---
...
- EmbeddedColumnChartBuilder is a builder specifically for column charts, offering various methods to customize chart appearance and data.
- You can add or remove data ranges, change chart types, and set options like background color, legend position, titles, and axis styles.
- The builder provides methods to modify the chart's position on the sheet and set strategies for handling hidden dimensions and merging multiple data ranges.
- The `build()` method finalizes the chart creation, which then needs to be inserted or updated in the spreadsheet.
- Advanced chart configuration options can be set using the `setOption()` method.
...
`EmbeddedColumnChartBuilder` — This builder, useful for chaining.
