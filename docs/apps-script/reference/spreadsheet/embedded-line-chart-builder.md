---
source: https://developers.google.com/apps-script/reference/spreadsheet/embedded-line-chart-builder
root: apps-script
fetched_at: 2026-04-23T15:21:38.673Z
---
...
- EmbeddedLineChartBuilder is a builder specifically for line charts in Google Visualization (Gviz).
- It provides methods to add and remove data ranges, set chart type, customize appearance (colors, background, titles, text styles), and control chart behavior (legend position, hidden dimensions, merge strategy, log scale, point and curve styles, category reversal, and row/column transposition).
- The build() method finalizes the chart configuration, but the resulting chart must be explicitly inserted or updated in the spreadsheet.
- The builder can also be used to change the chart type to other embedded chart types like AreaChart, BarChart, ColumnChart, ComboChart, HistogramChart, PieChart, ScatterChart, and TableChart, returning the corresponding builder for chaining.
- You can set advanced options for the chart using the setOption(option, value) method, although it doesn't validate the options or values provided.
