...
Replaces this image with an image described by a [`BlobSource`](../base/blob-source.md) object.

Inserting the image fetches it from the [`BlobSource`](../base/blob-source.md) once and a copy is stored for display inside the presentation. Images must be less than 50MB in size, cannot exceed 25 megapixels, and must be in either in PNG, JPEG, or GIF format.
