---
source: https://developers.google.com/apps-script/reference/slides/image.html
root: apps-script
fetched_at: 2026-04-23T15:21:05.400Z
---
...
`PageElement|null` — The parent page element of this image placeholder, or `null` if this image is not a placeholder or doesn't have a parent.
...
`Integer|null` — This image's placeholder index, or `null` if the image isn't a placeholder.
...
Replaces this image with an image described by a [`BlobSource`](https://developers.google.com/apps-script/reference/base/blob-source) object.

Inserting the image fetches it from the [`BlobSource`](https://developers.google.com/apps-script/reference/base/blob-source) once and a copy is stored for display inside the presentation. Images must be less than 50MB in size, cannot exceed 25 megapixels, and must be in either in PNG, JPEG, or GIF format.
