---
source: https://developers.google.com/apps-script/reference/slides/page-element.html
root: apps-script
fetched_at: 2026-04-23T15:21:10.115Z
---
...
- PageElement represents a visual element rendered on a page.
- PageElement provides methods to align, duplicate, remove, and manage the position, size, rotation, title, description, and transform of the element on the page.
- PageElement can be cast to specific types such as Group, Image, Line, Shape, SheetsChart, SpeakerSpotlight, Table, Video, and WordArt using corresponding `as...()` methods.
- You can get information about the PageElement including its connection sites, dimensions, position, unique ID, element type, parent group or page, and transform.
- Methods like `select()` allow interaction with the PageElement in the active presentation.
...
Pass `false` to select multiple `PageElement` objects. The `PageElement` objects must be in the same .

The following conditions must be met while selecting a page element using a `false` parameter:

1. The parent of the `PageElement` object must be the `current page selection`.
2. There should not be multiple objects selected.

To make sure that’s the case the preferred approach is to select the parent first using and then select the page elements in that page.
