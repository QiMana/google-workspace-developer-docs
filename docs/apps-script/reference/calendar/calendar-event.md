...
root: apps-script
fetched_at: 2026-04-23T15:18:51.934Z
---
...
[`Error`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error) — if this event is not an all-day event
...
For [all-day events](#isAllDayEvent\(\)), `getAllDayEndDate()` should almost always be called in preference to this method.
...
Gets the unique iCalUID of the event. Note that the iCalUID and the event [`id`](https://developers.google.com/calendar/v3/reference/events#resource-representations) used by the [Calendar v3 API](https://developers.google.com/calendar/v3/reference) and [Calendar advanced service](https://developers.google.com/apps-script/advanced/calendar) are not identical and cannot be used interchangeably. One difference in their semantics is that in recurring events all occurrences of one event have different `ids` while they all share the same iCalUIDs.
...
For [all-day events](#isAllDayEvent\(\)), `getAllDayStartDate()` should almost always be called in preference to this method.
...
| Name | Type | Description |
| --- | --- | --- |
| `startDate` | `Date` | the date when the event starts (the time is ignored) |
| `endDate` | `Date` | the date when the event ends (the time is ignored) |
...
| Name | Type | Description |
| --- | --- | --- |
| `startTime` | `Date` | the new start of the event |
| `endTime` | `Date` | the new end of the event |
