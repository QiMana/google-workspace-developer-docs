...
root: apps-script
fetched_at: 2026-04-23T15:18:54.535Z
---
...
| `timeZone` | `String` | the time zone to set the calendar to, specified in "long" format (e.g., "America/New\_York", as listed by [Joda.org](http://joda-time.sourceforge.net/timezones.html)) |
...
`Calendar|null` — the calendar with the given ID, or `null` if the calendar does not exist, if the user cannot access it, or if the user is not subscribed to the calendar
...
`Calendar|null` — the calendar with the given ID, or `null` if the calendar does not exist or the user does not own it
...
[`Error`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error) — if no calendar with this ID exists
