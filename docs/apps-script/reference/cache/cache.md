...
root: apps-script
fetched_at: 2026-04-23T15:18:50.379Z
---
...
- Data can be added to the cache using `put` or `putAll` methods, with optional expiration times.
- Cached values can be retrieved using the `get` or `getAll` methods.
- Entries can be removed from the cache using the `remove` or `removeAll` methods.
...
You still need to wait the 20 seconds if the item is not in the cache, but subsequent calls are very fast until the item expires out of the cache in 25 minutes.
...
Similar to repeated calls to "put", but more efficient as it only makes one call to the amount of data that can be stored per key is 100KB. The values expire from the cache after 600 seconds (10 minutes).
