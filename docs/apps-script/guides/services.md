...
Google Apps Script provides more than 30 built-in services for interacting with user data, other Google systems, and external systems. These services are provided as global objects akin to JavaScript's standard [`Math`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math) object. For example, just as `Math` offers methods like `random()` and constants like `PI`, Apps Script's [Spreadsheet service](../reference/spreadsheet.md) offers methods like [`openById(id)`](../reference/spreadsheet/spreadsheet-app.md#openById(String)), classes (child objects) like [`Range`](../reference/spreadsheet/range.md), and enums like [`DataValidationCriteria`](../reference/spreadsheet/data-validation-criteria.md).
...
The [V8 runtime](./v8-runtime.md) supports modern [ECMAScript](https://en.wikipedia.org/wiki/ECMAScript) syntax and features. The Rhino runtime is based on the older [JavaScript 1.6](https://developer.mozilla.org/en-US/docs/Web/JavaScript/New_in_JavaScript/1.6) standard, plus a few features from [1.7](https://developer.mozilla.org/en-US/docs/Web/JavaScript/New_in_JavaScript/1.7) and [1.8](https://developer.mozilla.org/en-US/docs/Web/JavaScript/New_in_JavaScript/1.8). [Choose which runtime](./v8-runtime.md#enabling_the_v8_runtime) to use with your script, but the V8 runtime is strongly recommended.

Each runtime supports JavaScript classes and objects that are available to your script in addition to the built-in and [advanced Google services](./services/advanced.md). Your scripts can use common objects like [`Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array), [`Date`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date), [`RegExp`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp), [and so forth](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference), as well as the [`Math`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math) and [`Object`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) global objects.

Because Apps Script code runs on Google's servers (with the exception of [HTML-service](./html.md) pages), browser-based JavaScript features like DOM manipulation or the [`Window`](https://developer.mozilla.org/en-US/docs/Web/API/Window) API are not available in Apps Script.
...
Each service provides at least one global (top-level) object; for example, the [Gmail service](../reference/gmail.md) is accessed solely from the [`GmailApp`](../reference/gmail/gmail-app.md) object. Some services provide multiple global objects; for example, the [Base service](../reference/base.md) includes four global objects: [`Browser`](../reference/base/browser.md), [`Logger`](../reference/base/logger.md), [`MimeType`](../reference/base/mime-type.md), and [`Session`](../reference/base/session.md).

## Methods

The global objects of nearly all built-in or [advanced services](./services/advanced.md) include methods that return data or an Apps Script class. Scripts make method calls in this format:

```
GlobalObjectName.methodName(argument1, argument2, ..., argumentN);
```

For example, a script can send an email by calling the [`sendEmail(recipient, subject, body)`](../reference/gmail/gmail-app.md#sendEmail(String,String,String)) method of the Gmail service like so:

```
GmailApp.sendEmail('claire@example.com', 'Subject line', 'This is the body.');
```

If a method returns another Apps Script class, chain method calls on one line. (Return types are shown both in autocomplete and in a method's reference documentation.) For example, the method [`DocumentApp.create()`](../reference/document/document-app.md#create(String)) returns a [`Document`](../reference/document/document.md); thus, the following two sections of code are equivalent:
...
Some services include classes labeled as "interfaces" in the reference documentation. These are generic classes used as return types for methods that can't determine the precise type in advance. For example, the [Document service](../reference/document.md) method [`Body.getChild(childIndex)`](../reference/document/body.md#getChild(Integer)) returns a generic [`Element`](../reference/document/element.md) object. The `Element` interface represents some other class, possibly a [`Paragraph`](../reference/document/paragraph.md) or [`Table`](../reference/document/table.md). Interface objects are rarely useful on their own; instead, call a method like [`Element.asParagraph()`](../reference/document/element.md#asParagraph()) to cast the object back to a specific class.

## Enums

Most services include enums (enumerated types) of named values. For example, the [Google Drive service](../reference/drive.md) uses the enums [`Access`](../reference/drive/access.md) and [`Permission`](../reference/drive/permission.md) to determine which users have access to a file or folder. In most cases, you access these enums from the global object, as shown in the following example:
