...
- [Messages](https://developers.google.com/workspace/add-ons/chat/send-messages) that can contain text, static or interactive cards, and buttons.
- [Dialogs](https://developers.google.com/workspace/add-ons/chat/dialogs) which are cards that open in a new window and typically prompt users to submit information.
- [Link previews](https://developers.google.com/workspace/add-ons/chat/preview-links) which are cards that preview information about an external service.

## Triggers

This section explains the triggers that Google Workspace add-ons use in Chat.

[Triggers](https://developers.google.com/workspace/add-ons/concepts/workspace-triggers) are the specific ways that users invoke a Chat app using the Chat UI, such as using @mentions or app commands.
...
Unlike other add-ons, you must configure any callback functions for these triggers using the Google Chat API. For guidance, see [Configure a Chat app](https://developers.google.com/workspace/add-ons/chat/configure).
...
To learn about add-on event objects within Chat and other Google Workspace applications, see [Event objects](https://developers.google.com/workspace/add-ons/concepts/event-objects).

## Chat actions

This section explains how Chat apps can use [add-on actions](https://developers.google.com/workspace/add-ons/concepts/actions) to respond to user interactions.
...
- [`DataActions`](https://developers.google.com/workspace/add-ons/reference/rpc/google.apps.card.v1#google.apps.card.v1.DataActions): Creates or updates Google Workspace data. To [send or update Chat messages](https://developers.google.com/workspace/add-ons/chat/send-messages), the object must contain the markup that defines the changes to the `Message` data, represented as [`chatDataActionMarkup`](https://developers.google.com/workspace/add-ons/reference/rpc/apps.extensions.markup#chatdataactionmarkup).
- [`RenderActions`](https://developers.google.com/workspace/add-ons/reference/rpc/google.apps.card.v1#renderactions): Create or Update a [dialog](https://developers.google.com/workspace/add-ons/chat/dialogs) or provide input suggestions for a [multi-select menu](https://developers.google.com/workspace/add-ons/chat/collect-information#add-multiselect).
- [`AuthorizationError`](https://developers.google.com/workspace/add-ons/guides/connect-third-party-service#build_and_return_a_sign-in_card): Prompts users with an authorization card to sign in or authenticate to a service that's external to Google. In Chat, only [basic authorization card](https://developers.google.com/workspace/add-ons/guides/connect-third-party-service#basic-auth-card) is supported.

The following table shows how Chat apps can respond with actions. Chat apps can return JSON objects or build the response using Apps Script's [`AddOnResponseService`](https://developers.google.com/apps-script/reference/add-ons-response-service/add-ons-response-service).
...
To learn about authenticating and calling the Chat API, see the [Chat API overview](https://developers.google.com/workspace/chat/api-overview).
