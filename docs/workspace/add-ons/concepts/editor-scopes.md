...
You can explicitly set the scopes your script project uses by editing its [manifest](https://developers.google.com/workspace/add-ons/concepts/editor-manifests) file. The manifest field [`oauthScopes`](https://developers.google.com/apps-script/manifest#Manifest.FIELDS.oauthScopes) is an array of all scopes used by the add-on. To set your project's scopes, do the following:

1. [View the scopes your add-on uses](#view_scopes). Determine what changes need to be made, such as using a narrower scope.
2. [Open your add-on's manifest file](https://developers.google.com/workspace/add-ons/concepts/editor-manifests#editing_a_manifest).
