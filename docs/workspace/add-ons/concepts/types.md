...
- **Build homepage experiences**: Create individual [homepages](https://developers.google.com/workspace/add-ons/concepts/homepages) for each Google Workspace app your add-on extends, or use the same homepage for multiple apps.
...
- **Use standardized interfaces**: Construct user interfaces from built-in widget elements provided by the Google Apps Script [`Card`](https://developers.google.com/apps-script/reference/card-service/card) service. You don't need expertise with HTML or CSS to define these interfaces.
...
Editor add-ons are ideal for automating tasks within Google Docs, Sheets, Slides, or Forms for individual or internal use. For add-ons requiring large-scale capabilities that need to handle many users, require low latency, or demands full control over your infrastructure, consider building a Google Workspace add-ons on a [different runtime](https://developers.google.com/workspace/add-ons/guides/alternate-runtimes) environment for better control over infrastructure, deployment and release processes.
