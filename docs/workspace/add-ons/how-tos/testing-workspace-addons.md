...
This page describes how to install an add-on that's under development (an *unpublished* or *developer* add-on) for testing or personal use. You can also debug the add-on using the Apps Script [debugger and breakpoints](https://developers.google.com/apps-script/guides/support/troubleshooting#use-debugger).

## Prerequisites

- Have editor access to the script project.
- To let others test the add-on, grant them editor access to the project. See [Collaborating with Other Developers](https://developers.google.com/apps-script/guides/collaborating).
...
After you install the add-on, it's immediately available in the host applications it extends. You might need to refresh the host application tab before the add-on appears. Authorize the add-on before you use it. If your project is already authorized, use [ScriptApp.invalidateAuth](https://developers.google.com/apps-script/reference/script/script-app#invalidateauth) to invalidate existing authorizations. This lets you test the [granular OAuth](https://developers.google.com/apps-script/concepts/scopes#handle-granular) feature.
...
When you test the add-on, follow the [Best practices](https://developers.google.com/workspace/add-ons/guides/workspace-best-practices). Also, do the following:
...
2. [Use example test data](https://developers.google.com/workspace/add-ons/guides/workspace-best-practices#_write_tests_and_test_messages) to evaluate your add-on's behavior.
3. If your add-on connects to a third-party API, verify that the service is accessible. Ensure that the add-on handles [authorization and sign-in](https://developers.google.com/workspace/add-ons/how-tos/non-google-services) correctly.
4. Handle error conditions gracefully. [Use error cards](https://developers.google.com/workspace/add-ons/guides/workspace-best-practices#_use_error_cards) where needed.
