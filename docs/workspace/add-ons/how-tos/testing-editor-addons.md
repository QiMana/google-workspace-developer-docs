...
Thoroughly test add-ons before you publish them to ensure they behave as intended. Apps Script lets you test [Editor add-ons](../concepts/types.md#editor_add-ons) in development on specific Google Docs Sheets Forms, or Slides files. Use testing to:

- Verify that an add-on in a standalone script functions as intended when applied to a sheet, doc, presentation, or form.
- Verify that the add-on installation flow works as intended, particularly for different initial [authorization lifecycle states](../concepts/editor-auth-lifecycle.md#the_complete_lifecycle) ([installed, enabled, or both](../concepts/editor-auth-lifecycle.md#installed_versus_enabled)).
...
7. In the **Config** section, select the initial [authorization state](../concepts/editor-auth-lifecycle.md#installed_versus_enabled) for the test.
...
The test document opens in a new tab. The add-on is in the [authorization state](../concepts/editor-auth-lifecycle.md#installed_versus_enabled) specified in the test deployment. Verify that the add-on functions as intended by interacting with its menu and UI elements.

To test the [granular OAuth](../../../apps-script/concepts/scopes.md#handle-granular) feature on your add-on, ensure that your project doesn't have authorizations. To invalidate any existing authorizations, use [ScriptApp.invalidateAuth](../../../apps-script/reference/script/script-app.md#invalidateauth).

### Test details

Keep the following in mind when you test Editor add-ons:

- [Installable triggers](../concepts/editor-triggers.md#installable_triggers_in_add-ons) aren't supported. Functionality that depends on installable triggers isn't testable.
...
- If your add-on uses the [Properties service](../../../apps-script/reference/properties/properties-service.md), properties persist and remain available the next time you run the test deployment.
