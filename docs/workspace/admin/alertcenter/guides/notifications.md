...
The Alert Center API can publish notifications to a Pub/Sub topic. After you create a topic and grant `alerts-api-push-notifications@system.gserviceaccount.com` publish permission, use the Alert Center API [update settings API](../reference/rest/v1beta1/TopLevel/updateSettings.md) to set that topic as a notification destination. Once configured, the Alert Center API publishes a message to that topic for each update, such as when a new alert arrives.
...
- `alertcenter_type`: The type of the alert. For a list of available alert types, see [Google Workspace Alert types](../reference/alert-types.md).
...
The payload of the message depends on `alertcenter_resource_type`. When the resource type is `"ALERT"`, the payload is one of the Alert Center API [alert types](../reference/alert-types.md). The format of the payload depends on `alertcenter_payload_format`. When the format is `"JSON"`, the payload is a [base64-encoded](https://en.wikipedia.org/wiki/Base64) [JSON](https://en.wikipedia.org/wiki/JSON) object.
