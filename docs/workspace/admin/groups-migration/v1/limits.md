---
source: https://developers.google.com/workspace/admin/groups-migration/v1/limits
root: workspace
fetched_at: 2026-04-23T15:23:55.640Z
---

# Groups Migration API: limits and quotas

Limits and quotas protect the Google infrastructure from an automated process that uses the Groups Migration API in an inappropriate way. Excessive requests from an API might result from a typo or an inefficiently designed system that makes needless API calls. Blocking traffic from a specific source once it reaches a certain level is necessary for the overall health of the Google Workspace system.

## API request failures

In the unlikely event that your API request fails, your application receives an HTTP status code response. An HTTP status code of `403` has error information about incorrect input, and an HTTP status code of `503` has error information indicating which API quotas have been exceeded. These responses let your custom application detect these errors and take appropriate action.
...
| Mail message size | The maximum mail message size is 25 MB. This limit includes the message's metadata headers, body, and any attachments. |
