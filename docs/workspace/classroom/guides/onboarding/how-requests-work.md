...
For specific code samples, see the corresponding API guides, for example [Create & manage courses](https://developers.google.com/workspace/classroom/guides/manage-courses) or [Create & manage coursework](https://developers.google.com/workspace/classroom/guides/manage-coursework).

## Resource-oriented design

As mentioned in [API resources](https://developers.google.com/workspace/classroom/guides/key-concepts/api-structure), the Classroom API follows [resource-oriented design](https://google.aip.dev/121) patterns. Most resources have methods for standard operations like creating, reading, updating, and deleting instances of the resource.

For example, it's possible to [`create()`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses/create), [`patch()`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses/patch), [`get()`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses/get), [`list()`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses/list), and [`delete()`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses/delete) a Classroom [`Course`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses) using the API.
...
For subresources (sometimes called child resources), identifiers for the parent resource are also required. For example, when creating a [`CourseWork`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses.courseWork) within a `Course`, the `Course` `id` is needed to establish in which `Course` the `CourseWork` belongs.
...
The `get()` method retrieves a specific resource by some identifier. For example, fetching a `Course` based on `id` or [`alias`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses.aliases). The `get()` call returns the full resource directly.
...
Not all operations possible with the Classroom API can be achieved with the aforementioned standard operations, for example, modifying the assignees of a `CourseWork` resource. In these cases, custom methods are available, like the [`modifyAssignees`](https://developers.google.com/workspace/classroom/reference/rest/v1/courses.courseWork/modifyAssignees) method. The behavior of these methods are bespoke and you should consult the documentation for them individually.
