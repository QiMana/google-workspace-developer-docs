...
1. In the third-party application, a teacher chooses the option to import a Classroom [Course](../reference/rest/v1/courses.md).
2. The third-party application calls the [`courses.list`](../reference/rest/v1/courses/list.md) method through the Classroom API, which will return a response JSON with all the teacher's courses.
...
4. With the selected course ID, the third-party application calls the [`students.list`](../reference/rest/v1/courses.students/list.md) and [`teachers.list`](../reference/rest/v1/courses.teachers/list.md) methods and displays all names on its website for teachers to confirm for import.
...
1. [Manage Courses with the Classroom API](../guides/manage-courses.md)
2. [Manage Students and Teachers](../guides/manage-users.md)
...
| Parameter | Use |
| --- | --- |
| courseState | Left unspecified, the API will return courses of all six [course states](../reference/rest/v1/courses.md#CourseState). We recommend specifying `ACTIVE` to retrieve the courses teachers are currently using. |
...
All available fields for the [student](../reference/rest/v1/courses.students.md) and [teacher](../reference/rest/v1/courses.teachers.md) resources can be found in their respective documentation. The two mostly commonly used and typically required needed fields are in the `profile` field: `profile.name` and `profile.emailAddress`.
...
To retrieve and use any of these course or roster details from Classroom, your application will need to request [authorization](../guides/auth.md) from users. There are three (3) required scopes to implement this workflow:
...
As the school year progresses, rosters might change as students drop or add courses. Adding [Pub/Sub notifications](../best-practices/push-notifications.md) will allow you to keep your third-party application in sync with Classroom rosters. To receive notifications, you set up a Google Cloud Pub/Sub topic and then register the topic with the Classroom API. This registration is a request for Classroom to send data from the given feed to the given topic. This feed will be the event triggers for re-syncing with a teacher's Classroom roster.
...
For more details on how to integrate with Classroom push notifications, see our [Manage Push Notifications guide](../best-practices/push-notifications.md).
