...
This page explains how to use the Google Workspace Events API to create a subscription to a Google Workspace resource. A Google Workspace subscription lets your app receive information about *Google Workspace events*, which represent changes to a Google Workspace resource. To learn about which resources and event types the Google Workspace Events API supports, see the [Google Workspace Events API overview](../../events.md).
...
3. Call the Google Workspace Events API's [`create`](../reference/rest/v1/subscriptions/create.md) method on the [`Subscription`](../reference/rest/v1/subscriptions.md#Subscription) resource.
...
- A [Google Cloud project](../../guides/create-project.md) with billing enabled. For subscriptions to Chat, you must also enable the Chat API in your Cloud project and configure the **App name**, **Avatar URL**, and **Description** fields. For details, see [Build a Google Chat app](../../chat/quickstart/gcf-app.md).
- Requires authentication and an [appropriate authorization scope for each event type](./auth.md#scopes-event-type) in the subscription:
	- Typically requires user authentication with the OAuth consent screen configured for the app. When you configure the consent screen, you must specify a scope to support each event type for the subscription. To configure the consent screen and identify required scopes, see [Choose scopes](./auth.md).
	...
	1. On your topic's page, go to the side panel and open the **Permissions** tab.
		...
		1. For Chat events:
			- If your Chat app is a Google Workspace add-on that extends Chat, use the service account email on the [Chat API configuration page](https://console.developers.google.com/apis/api/chat.googleapis.com/hangouts-chat). To learn more, see [Build a Google Chat app that uses Pub/Sub](../../add-ons/chat/quickstart-pubsub.md).
						- If your Chat app uses [Chat API interaction events](../../chat.md), use `chat-api-push@system.gserviceaccount.com`.
...
In a Google Workspace subscription, the *target resource* is the Google Workspace resource that you monitor for events. The target resource is represented in the subscription's [`targetResource`](../reference/rest/v1/subscriptions.md#Subscription.FIELDS.target_resource) field, formatted using the full resource name. For example, for a subscription that monitors a Chat space (`spaces/AAAABBBBBBB`), the value of the `targetResource` is `//chat.googleapis.com/spaces/AAAABBBBBBB`.
...
| --- | --- | --- |
| File | `//drive.googleapis.com/files/FILE`  where FILE is the ID in the [resource name](../../drive/api/reference/rest/v3/files.md#File.FIELDS.drive_id) of the Drive API `files` resource. You can obtain the ID from the file's URL or by using the [`files.list`](../../drive/api/reference/rest/v3/files/list.md) method. | The user that authorizes the subscription must have permission on the file within the subscription relative to the subscribe to event. |
| Shared drive | `//drive.googleapis.com/drives/DRIVE`  where DRIVE is the ID in the [resource name](../../drive/api/reference/rest/v3/drives.md#Drive.FIELDS.id) of the Drive API `drives` resource. You can obtain the ID from the drive's URL or by using the [`drives.list`](../../drive/api/reference/rest/v3/drives/list.md) method. | The subscription only receives events for items in the shared drive where the user is a member through their Google Workspace Account or Google Account. |
...
To create a subscription, use the Google Workspace Events API's `subscriptions.create` method to create a [`Subscription`](../reference/rest/v1/subscriptions.md) resource. Specify the following fields:
...
1. In your Apps Script project, create a new script file named `createSubscription` and add the following code:
	...
	```
	Replace the following:
	- `TARGET_RESOURCE`: The [Google Workspace resource](../reference/rest/v1/subscriptions.md#Subscription.FIELDS.target_resource) that you're subscribing to, formatted as its full resource name. For example, to subscribe to a Chat space with the space ID `AAAABBBB`, use `//chat.googleapis.com/spaces/AAAABBBB`.
		- `EVENT_TYPES`: One or more [event types](../reference/rest/v1/subscriptions.md#Subscription.FIELDS.event_types) that you want to subscribe to in the target resource. Format as an array of strings such as `'google.workspace.chat.message.v1.created'`.
		...
		- `True`: Includes all resource data. To limit which fields are included, add the [`fieldMask`](../reference/rest/v1/subscriptions.md#PayloadOptions.FIELDS.field_mask) field and specify at least one field for the changed resource. Only subscriptions to Chat resources support including resource data.
...
The Google Workspace Events API returns a completed [long-running operation](../reference/rest/v1/operations.md) that contains the instance of the `Subscription` resource that you created.
...
For each Google Workspace event that you triggered, a message is delivered to your Pub/Sub subscription that contains the event. For details, see [Receiving events as Google Cloud Pub/Sub messages](../../events.md#pub-sub).
...
Here's how to create a subscription to Chat space events with [app authentication and administrator approval](../../chat/authenticate-authorize-chat-app.md):

1. If necessary, [Build a Chat app](../../chat/quickstart/gcf-app.md).
...
- `SPACE_ID`: The space name, which you can obtain from the [`spaces.list` method](../../chat/api/reference/rest/v1/spaces/list.md) in the Chat API, or from a space's URL.
- `TOPIC_NAME`: The full name of the Pub/Sub topic that you created in your Cloud project. Formatted as `projects/PROJECT_ID/topics/TOPIC_ID`.
- `RESOURCE_DATA`: A boolean that specifies whether the subscription includes resource data in the payload:
	- `True`: Includes all resource data. To limit which fields are included, add the [`fieldMask`](../reference/rest/v1/subscriptions.md#PayloadOptions.FIELDS.field_mask) field and specify at least one field for the changed resource. Only subscriptions to Chat resources support including resource data.
		- `False`: Excludes resource data.
