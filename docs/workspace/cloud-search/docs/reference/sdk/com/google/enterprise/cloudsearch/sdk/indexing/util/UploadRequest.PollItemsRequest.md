---
source: https://developers.google.com/workspace/cloud-search/docs/reference/sdk/com/google/enterprise/cloudsearch/sdk/indexing/util/UploadRequest.PollItemsRequest
root: workspace
fetched_at: 2026-04-23T15:26:57.070Z
---

# UploadRequest.PollItemsRequest

Request to poll items from the indexing API.

```
limit specifies the maximum number of items to return.
statusCodes is a list of statuses of Item that will be polled from the
indexing API. Valid status codes are {ERROR, MODIFIED, NEW_ITEM, ACCEPTED}.
queue is the queue name to fetch items from. If unspecified, it will fetch from
"default" queue.
```

An example of PollItemsRequest is:
