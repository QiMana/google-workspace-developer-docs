...
| Object type | **`itemMetadata.objectType.field=type`**   **`` itemMetadata.objectType.defaultValue=`movie` ``**   The object type used by the connector, as defined in the [schema](../guides/schema-guide.md). The connector won't index any structured data if this property is not specified.   If the `objectType.field` isn't set to a value, the value for `objectType.defaultValue` is used. |
...
| Partition size | `**traverse.partitionSize**=batchSize`  Number of [`ApiOperation()`](./sdk/com/google/enterprise/cloudsearch/sdk/indexing/template/ApiOperation.md) to be processed in batches before fetching additional `APIOperation`.  The default value is `50`. |
