...
When you use the [`documents.get`](../reference/rest/v1/documents/get.md) method to fetch document content, the content might include unresolved suggestions. To control how `documents.get` represents suggestions, use the optional [`SuggestionsViewMode`](../reference/rest/v1/documents.md#SuggestionsViewMode) parameter. The following filter conditions are available with this parameter:
...
Unlike text insertions or deletions, these don't offset the indexes—although they might break up a [`TextRun`](../reference/rest/v1/documents.md#textrun) into smaller chunks—but just add annotations about the suggested style change.

One such annotation is a [`SuggestedTextStyle`](../reference/rest/v1/documents.md#suggestedtextstyle), which consists of 2 parts:
