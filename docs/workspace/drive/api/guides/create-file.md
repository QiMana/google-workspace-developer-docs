...
To create a file in Drive that contains no metadata or content, use the [`create`](../reference/rest/v3/files/create.md) method on the [`files`](../reference/rest/v3/files.md) resource with no parameters.

When you create the file, the method returns a `files` resource. The file is given a `kind` of `drive.file`, an `id`, a `name` of "Untitled", and a `mimeType` of `application/octet-stream`. The [`uploadType`](../reference/rest/v3/files/create.md#body.QUERY_PARAMETERS.upload_type) is marked as required but defaults to `media`, so you don't actually have to supply it.

For more information about Drive file limits, see [File and folder limits](./folder.md#folder-limits).
...
- **User account (OAuth 2.0)**: If the application authenticates on behalf of a user, that user becomes the file owner. The file then resides in their My Drive folder or a [specified folder](./folder.md#create-file). It consumes their storage quota.
...
For more information about file permissions, see [Share files, folders, and drives](./manage-sharing.md).

## Generate IDs to use with your files

The [`generateIds`](../reference/rest/v3/files/generateIds.md) method on the [`files`](../reference/rest/v3/files.md) resource lets you pre-generate unique file IDs that can be used when creating or copying files and folders in Drive. This can be useful when you need to control the file IDs from your app, rather than letting Drive assign them automatically.

You can set the number of IDs generated using the [`count`](../reference/rest/v3/files/generateIds.md#body.QUERY_PARAMETERS.count) query parameter. If `count` is not set, 10 are returned by default. The maximum number of IDs you can request is capped at 1,000.

You can also designate the [`space`](../reference/rest/v3/files/generateIds.md#body.QUERY_PARAMETERS.space) in which the IDs can be used and the [`type`](../reference/rest/v3/files/generateIds.md#body.QUERY_PARAMETERS.type) of items which the IDs can be used for.
...
One example of a metadata-only file is a folder with the MIME type `application/vnd.google-apps.folder`. For more information, see [Create and populate folders](./folder.md). Another example is a shortcut that points to another file on Drive with the MIME type `application/vnd.google-apps.shortcut`. For more information, see [Create a shortcut to a Drive file](./shortcuts.md).

## Manage thumbnail images

Thumbnails help users identify Drive files. Drive can automatically generate thumbnails for common file types or you can provide a thumbnail image generated by your app. For more information, see [Upload thumbnails](./file-metadata.md#upload-thumbnails).

## Copy an existing file

To copy a file, and apply any requested updates, use the [`copy`](../reference/rest/v3/files/copy.md) method on the `files` resource. To find the `fileId` to copy, use the [`list`](../reference/rest/v3/files/list.md) method.
...
You can pre-set the file ID of the copied file using the [`generateIds`](../reference/rest/v3/files/generateIds.md) method. For more information, see [Generate IDs to use with your files](#generate-ids).

Note that you need to use an appropriate [Drive API scope](../reference/rest/v3/files/copy.md#authorization-scopes) to authorize the call. For more information on Drive scopes, see [Choose Google Drive API scopes](./api-specific-auth.md).
...
- **Permissions**:
	- The [`DownloadRestrictionsMetadata`](../reference/rest/v3/files.md#downloadrestrictionsmetadata) object of the [`files`](../reference/rest/v3/files.md) resource determines who can copy the file. For more information, see [Prevent users from downloading, printing, or copying your file](./content-restrictions.md#download-print-copy).
		- The [`capabilities.canCopy`](../reference/rest/v3/files.md#File.FIELDS.inlinedField_8) field resource determines whether the user can copy a file. For more information, see [Understand file capabilities](./manage-sharing.md#capabilities).
...
- **File management**:
	- Some files, like [third-party shortcuts](./third-party-shortcuts.md), can never be copied.
		- You can only copy a file into one parent folder. Specifying multiple parents isn't supported. If the [`parents`](../reference/rest/v3/files.md#File.FIELDS.parents) field isn't specified, the file inherits any discoverable parents from the source file.
		...
		- Excessive use of `copy` can lead to exceeding your Drive API quota limits. For more information, see [Usage limits](./limits.md).
...
- To create a file in a specific folder, see [Create a file in a specific folder](./folder.md#create-file).
- To move files, see [Move files between folders](./folder.md#move-files).
- To work with file metadata, see [Manage file metadata](./file-metadata.md).
- To delete a file, see [Trash or delete files and folders](./delete.md).
