...
When a file moves between user corpora it may also appear `deleted` even though the user still retains access to the file. If you query change logs for multiple corpora, use the `includeCorpusRemovals` parameter in [Changes.list](https://developers.google.com/workspace/drive/api/reference/rest/v3/changes/list) to disambiguate corpus moves from loss of access. For definitions of different corpora, see [Files and folders overview](https://developers.google.com/workspace/drive/api/guides/about-files).
