...
This document contains Drive Labels API-specific authorization and authentication information. Before reading this document, be sure to read the Google Workspace's general authentication and authorization information at [Learn about authentication and authorization](../../../guides/auth-overview.md).
...
For more information, see [Drive API OAuth 2.0 Scopes](https://developers.google.com/identity/protocols/oauth2/scopes#drive) and [Drive API-specific authorization](../../api/guides/api-specific-auth.md).
...
| Scope | Meaning |
| --- | --- |
| `https://www.googleapis.com/auth/drive.labels` | View, use, and manage Drive labels. - This scope allows user access, and is required for write operations when `use_admin_access` is not set. - Write operations with this scope are valid for labels with the `SHARED` [label type](./overview.md#label-type). |
...
| `https://www.googleapis.com/auth/drive.admin.labels` | View, edit, create, and delete all Drive labels in your organization, and view your organization's label-related administration policies. - This scope allows admin access, and is required for write operations when `use_admin_access` is set. This scope also grants read access. - Write operations with this scope are valid for labels with the `SHARED` or `ADMIN` [label type](./overview.md#label-type). |
...
When `useAdminAccess` is not set, write requests are allowed only for labels with both a `SHARED` [`label type`](./overview.md#label-type) and for users with the appropriate `EDITOR` role for this label.

For more information, see [Create access credentials](../../../guides/create-credentials.md).
