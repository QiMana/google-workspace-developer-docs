...
You can update a single label by applying a set of update requests resulting in a new draft revision. For example, you can update an existing label [property](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels#properties_1) (the label description) or you can add a new [`Field`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels#field) to the label.

The label update acts as a batch update as the method takes a list of update [`Request`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta#request) objects. It makes updates according to the same order specified in the batch request. The updates in the batch update are applied atomically. That is, if any request in the batch is unsuccessful, then the entire update is unsuccessful and none of the (potentially dependent) changes are applied. The label is left unchanged.
...
The following operations supported by the [`labels.delta`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta) method can be grouped into the following broad categories:
...
The [`labels.delta`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta) method works by taking one or more [Request](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta#request) objects, each one specifying a single type of request to perform. There are many different kinds of requests. Here's a breakdown on the types of requests, grouped into different categories.
...
To update a label, use the [`delta`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta) method on the [`labels`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels) collection.
...
- A [`Request`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels/delta#Request) that specifies the applicable updates to the label.
...
- One or more [`Field`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels#field).
...
The label, field, or choice is updated, the label's revision ID is incremented, and the label is stored in a database as a draft label. The label has the [`State`](https://developers.google.com/workspace/drive/labels/reference/rest/v2/labels#state) of `PUBLISHED` with `hasUnpublishedChanges=true` meaning there are draft changes, but they aren't available to users. The updated label must be `PUBLISHED` before the changes become visible to users. For more information, see [Label lifecycle](https://developers.google.com/workspace/drive/labels/guides/label-lifecycle).
