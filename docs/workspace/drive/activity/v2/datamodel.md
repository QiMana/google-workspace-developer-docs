...
- [`DriveActivity`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/driveactivity): This is the primary resource returned by queries to the Drive Activity API. It describes one or more actors performing one or more actions affecting one or more targets.
- [`Timestamp`](https://protobuf.dev/reference/protobuf/google.protobuf/#timestamp) and [`TimeRange`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/timerange): These describe, respectively, either a single point in time when the activity occurred, or the start and end of when the activity occurred over a span of time.
- [`Actor`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/actor): Typically, an `Actor` is an end user. However, sometimes, a system event can trigger an [`Action`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/action) when an administrator is acting as a user or as themselves, or when performed by an unidentifiable person. The `Actor` message encapsulates each of these cases.
- [`Target`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/target): A `Target` is the object of an activity, like a file, a folder, a shared drive, or a file comment. Note that many action types support more than one kind of target. For example, though `Edit` generally applies to Drive files, other actions like `Rename` and `Create` can also apply to Drive folders and shared drives. Targets that aren't Drive items can still refer to one, such as the root folder of a drive or the parent document containing a file comment.
- [`Action`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/action): Each `DriveActivity` resource has one or more related actions. An `Action` is self-contained, like an *event*, in that it comprises not only the detailed type and information about the action, but also an `Actor`, a `Target`, and either a `Timestamp` or `TimeRange`. To avoid redundancy, an `Action` doesn't populate its own `Target`, `Actor`, or time fields when those are the same as the overall `DriveActivity`.
- [`ActionDetail`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/actiondetail): This is the specific type and detailed information about an `Action`. For example, a `Move` action detail has a source and destination location, and a `PermissionChange` specifies who can now access a document and with what privileges.
...
When a [`ConsolidationStrategy`](https://developers.google.com/workspace/drive/activity/v2/reference/rest/v2/activity/query#consolidationstrategy) is used, related actions are grouped into one combined `DriveActivity`. In this example, two similar actions are grouped: one `Edit` action type from two different users.
