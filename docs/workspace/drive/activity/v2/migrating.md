...
"Grouping" is now called [Consolidation](./requests.md#consolidation), and the `source` and `userId` request parameters have been removed.

There are also new [Filter](./requests.md#filters) options that allow you to constrain the types of activity data returned in the response.
...
In the v2 API, a richer set of [`Actor`](./reference/rest/v2/activity/actor.md) types is available, and `user.knownUser` is populated when the actor is a known user. If your application needs detailed information about users, it can query it from the [People API](https://developers.google.com/people) by passing the `KnownUser` field [`personName`](./reference/rest/v2/activity/user.md#knownuser) to the [`people.get`](https://developers.google.com/people/api/rest/v1/people/get) method.

## Targets

In the v1 API, targets were always Drive items. In the v2 API, targets can be other types of objects in Drive. For example, changes to a drive have a target type of [`Drive`](./reference/rest/v2/activity/drive.md). The root folder of a shared drive is still returned (as a [`DriveItem`](./reference/rest/v2/activity/driveitem.md) in the `root` field), but it's not the immediate target of the activity. A similar concept applies to a [`FileComment`](./reference/rest/v2/activity/filecomment.md) resource, whose `parent` field refers to the Drive item containing the target comment thread.
...
In the v2 API, the style of response doesn't change based on the consolidation strategy, as the returned [`DriveActivity`](./reference/rest/v2/activity/driveactivity.md) always contains the full set of actors, targets, and actions.
