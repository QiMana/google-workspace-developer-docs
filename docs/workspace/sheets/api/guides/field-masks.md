...
Spreadsheets can be large, and often you don't need every part of the [`Spreadsheet`](../reference/rest/v4/spreadsheets.md#resource:-spreadsheet) resource returned by a read request. You can limit what's returned in a Sheets API response, using the `fields` URL parameter. For best performance, [explicitly list only the fields you need](./performance.md#partial-response) in the reply.
...
The following [`spreadsheets.get`](../reference/rest/v4/spreadsheets/get.md) request example uses a field mask of `sheets.properties(sheetId,title,sheetType,gridProperties)` to fetch only the sheet ID, title, [`SheetType`](../reference/rest/v4/spreadsheets/sheets.md#SheetType), and [`GridProperties`](../reference/rest/v4/spreadsheets/sheets.md#GridProperties) of a [`SheetProperties`](../reference/rest/v4/spreadsheets/sheets.md#SheetProperties) object on all sheets in a spreadsheet:

```
GET https://sheets.googleapis.com/v4/spreadsheets/spreadsheetId?fields=sheets.properties(sheetId,title,sheetType,gridProperties)
```

The response to this method call is a [`Spreadsheet`](../reference/rest/v4/spreadsheets.md#resource:-spreadsheet) object containing the components requested in the field mask. Note that `sheetType=OBJECT` doesn't contain `gridProperties`:
...
Sometimes you need to update only certain fields in an object while leaving the other fields unchanged. Update requests inside a [`spreadsheets.batchUpdate`](../reference/rest/v4/spreadsheets/batchUpdate.md) operation use field masks to tell the API which fields are being changed. The update request ignores any fields that aren't specified in the field mask, leaving them with their current values.
...
The following example uses the [`AddSheetRequest`](../reference/rest/v4/spreadsheets/request.md#addsheetrequest) to add a new sheet of type `Grid`, freeze the first row, and color the new sheet's tab red:
