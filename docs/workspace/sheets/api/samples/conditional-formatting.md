...
These examples are presented as HTTP requests to be language neutral. To learn how to implement a batch update in different languages using the Google API client libraries, see [Update spreadsheets](../guides/batchupdate.md#example).
...
The following [`spreadsheets.batchUpdate`](../reference/rest/v4/spreadsheets/batchUpdate.md) method code sample shows how to use the [`AddConditionalFormatRuleRequest`](../reference/rest/v4/spreadsheets/request.md#addconditionalformatrulerequest) to establish new gradient conditional formatting rules for rows 10 and 11 of a sheet. The first rule states that cells in that row have their background colors set according to their value. The lowest value in the row is colored dark red, while the highest value is colored bright green. The color of the other values is interpolated. The second rule does the same, but with specific numeric values determining the gradient endpoints (and different colors). The request uses the [`sheets.InterpolationPointType`](../reference/rest/v4/spreadsheets/sheets.md#interpolationpointtype) as the `type`.
...
The following [`spreadsheets.batchUpdate`](../reference/rest/v4/spreadsheets/batchUpdate.md) method code sample shows how to use the [`DeleteConditionalFormatRuleRequest`](../reference/rest/v4/spreadsheets/request.md#deleteconditionalformatrulerequest) to delete the conditional formatting rule with index `0` in the sheet specified by SHEET\_ID.
...
The following [`spreadsheets.get`](../reference/rest/v4/spreadsheets/get.md) method code sample shows how to get the title, SHEET\_ID and list of all conditional formatting rules for each sheet in a spreadsheet. The `fields` query parameter determines what data to return.
...
The response consists of a [`Spreadsheet`](../reference/rest/v4/spreadsheets.md) resource, which contains an array of [`Sheet`](../reference/rest/v4/spreadsheets/sheets.md) objects each having a [`SheetProperties`](../reference/rest/v4/spreadsheets/sheets.md#sheetproperties) element and an array of [`ConditionalFormatRule`](../reference/rest/v4/spreadsheets/sheets.md#conditionalformatrule) elements. If a given response field is set to the default value, it's omitted from the response. The request uses the [`ConditionType`](../reference/rest/v4/spreadsheets/other.md#conditiontype) as the `type` for the [`BooleanRule`](../reference/rest/v4/spreadsheets/sheets.md#booleanrule).
