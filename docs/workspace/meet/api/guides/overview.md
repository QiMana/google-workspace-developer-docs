...
- [Respond to events from Google Meet](./events-overview.md)
- [Subscribe to events using the Google Workspace Events API](../../../events.md)
...
You can also incorporate other Google Workspace APIs like Chat API into your app to take the collaboration even further. For more information, see [Chat use cases](../../../chat/overview.md#use-cases).
...
A file generated by Meet in response to a *conference*, such as [`recordings`](../reference/rest/v2/conferenceRecords.recordings.md) and [`transcripts`](../reference/rest/v2/conferenceRecords.transcripts.md). Artifacts are saved to the *meeting organizer's* Google Drive. They're usually ready to be fetched soon after a conference ends.
...
A unique ID for each instance of a *conference*. It's created automatically when the conference starts. The [`conferenceRecords`](../reference/rest/v2/conferenceRecords.md) resource uses the format `conferenceRecords/{conferenceRecord}`. For example, `conferenceRecords/kRyYx8b7vNDsLpR1tG_cNjFUQBoBRhHIMoGJAJkBCQ`.
...
A unique server-generated ID used to identify a *meeting space*. It's case sensitive. For example, `jQCFfuBOdN5z`. The meeting ID is returned in the `name` field of a [`spaces`](../reference/rest/v2/spaces.md) resource. For more information, see [How Meet identifies a meeting space](./meeting-spaces-overview.md#identify-meeting-space).
...
- To learn how to create a meeting space with Meet REST API, read the [Create and manage meeting spaces](./meeting-spaces.md) guide.
- To learn how to subscribe to Google Workspace events, see [Subscribe to events using the Google Workspace Events API](../../../events.md).
