...
1. All [origins](./overview.md#origin) used in the operation of your add-on must use `https` as the protocol.
2. The `addOnOrigins` field in the [add-on manifest](./deploy-add-on.md#create-deployment) must be populated with the origins that your add-on is using.
	...
	- Set the [`frame-src`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-src) value of the iframes containing your application.
		...
		- The `sidePanelUri` field in the add-on manifest. For more information, see [Deploy a Meet add-on](./deploy-add-on.md#create-deployment).
				- The `sidePanelUrl` and `mainStageUrl` properties in the [`AddonScreenshareInfo`](../reference/websdk/screenshare_api.addonscreenshareinfo.md.md) object. For more information, see [Promote an add-on to users through screen sharing](./promote.md#screen_sharing).
				- The `sidePanelUrl` and `mainStageUrl` properties in the [`ActivityStartingState`](../reference/websdk/addon_sdk.activitystartingstate.md.md). For more information on activity starting state, see [Collaborate using a Meet add-on](./collaborate-in-the-add-on.md).
		- Validate the origin of the site that's calling the [`exposeToMeetWhenScreensharing()`](../reference/websdk/screenshare_api.meetaddonscreenshare.exposetomeetwhenscreensharing.md.md) method.
