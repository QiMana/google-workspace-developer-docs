...
To use an add-on in Meet, you need a deployment and an [add-on manifest file](https://developers.google.com/apps-script/add-ons/concepts/workspace-manifests).
...
3. Click **Next**.
	...
	The [add-on manifest file](https://developers.google.com/apps-script/add-ons/concepts/workspace-manifests) is the central configuration for a Google Meet add-on. The following code sample shows the available Meet fields for web in the add-on manifest file.
	```
	{
	  "addOns": {
	    "common": {
	...
	- NAME: String. The name of your Google Meet add-on.
		...
		- SIDE\_PANEL\_URL: String. The URL to the entry point of your add-on app. This is displayed in an iframe within the [side panel](https://developers.google.com/workspace/meet/add-ons/guides/overview#side-panel). The [origin](https://developers.google.com/workspace/meet/add-ons/guides/overview#origin) of this URL must be part of the origins specified in the ADD\_ON\_ORIGINS field.
		...
		- ADD\_ON\_ORIGINS: List of strings. A list of [origins](https://developers.google.com/workspace/meet/add-ons/guides/overview#origin) where your add-on is hosted. Two URLs have the same origin when they share the same scheme, host, and port. Sub origins are also permitted, as are wildcard subdomains. For more information, see [Add-on security](https://developers.google.com/workspace/meet/add-ons/guides/add-on-security).
		- MEET\_WEB\_LOGO\_URL: Optional. String. A Meet-specific URL of the logo for the add-on. This logo is used throughout Meet. If not present, the `logoUrl` from the common section is used. For logo design guidelines, see [Best practices](https://developers.google.com/workspace/meet/add-ons/guides/best-practices#logo-design).
		- DARK\_MODE\_LOGO\_URL: String. A dark mode specific URL of the logo for the add-on. Supplying a dark mode logo makes sure your add-on will look best in any Meet theme. For logo design guidelines, see [Best practices](https://developers.google.com/workspace/meet/add-ons/guides/best-practices#logo-design).
4. Click **Submit**.
	For more information on deployments, see [Create a deployment resource](https://developers.google.com/workspace/add-ons/guides/alternate-runtimes#create_a_deployment_resource).
...
3. Click **Save**.
	For details on how to create an Apps Script project, see [Apps Script documentation](https://developers.google.com/apps-script/overview). The Meet add-on relies solely on the appsscript.json manifest file, also called the [Apps Script project manifest](https://developers.google.com/apps-script/concepts/manifests). Make sure the manifest file in your Apps Script project contains an `addOns` and a `meet` section.
...
1. Follow the Google Workspace add-on documentation to [Install an unpublished add-on](https://developers.google.com/workspace/add-ons/how-tos/testing-workspace-addons#install_an_unpublished).
...
In addition to installing your add-on for the individual signed-in user, you can also [publish it](https://developers.google.com/workspace/meet/add-ons/guides/publish). When you publish your Google Workspace add-on, you make it available for others to find, install, and use.
