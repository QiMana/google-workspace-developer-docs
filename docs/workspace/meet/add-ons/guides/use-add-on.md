...
To allow users to collaborate together, the add-on can set the [`ActivityStartingState`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.activitystartingstate). For more information, see [Collaborate using a Meet add-on](https://developers.google.com/workspace/meet/add-ons/guides/collaborate-in-the-add-on).

After opening an add-on, side panel users can start working together once the add-on invokes the [`startActivity()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetaddonclient.startactivity) method.
...
Collaborative activities can be done in the side panel or the main stage. If a `mainStageUrl` is listed in the add-on manifest, invoking the [`startActivity()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetaddonclient.startactivity) method automatically opens the add-on in the main stage and starts the activity.

Once the main stage is open, an add-on might choose to close the side panel by calling the [`unloadSidePanel()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetmainstageclient.unloadsidepanel) method. To extend the earlier example, a side panel that chooses a document may no longer be relevant and can be closed.

To re-open the side panel view, the add-on can call the [`loadSidePanel()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetmainstageclient.loadsidepanel) method. For example, an add-on that creates user polls might reopen the side panel to display question response rates.

## Start an add-on from screen sharing

Users can start add-ons while screen sharing. If a user is screen sharing a website that has an add-on, that website can promote its add-on to the user by using the [`exposeToMeetWhenScreensharing()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/screenshare_api.meetaddonscreenshare.exposetomeetwhenscreensharing) method. This shows the user a notification banner in the Meet call, prompting the user to either install or start the add-on. For more information, see [Promote an add-on to users through screen sharing](https://developers.google.com/workspace/meet/add-ons/guides/promote#screen_sharing).
...
- [Promote an add-on to users through screen sharing](https://developers.google.com/workspace/meet/add-ons/guides/screen-sharing)
