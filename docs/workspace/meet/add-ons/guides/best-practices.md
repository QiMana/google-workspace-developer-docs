...
- **When using gstatic**: The latest SDK version is contained within the gstatic URL found in [the instructions for using the SDK](https://developers.google.com/workspace/meet/add-ons/guides/use-SDK).
...
We strongly encourage setting up your add-on based on user actions taken in the [side panel](https://developers.google.com/workspace/meet/add-ons/guides/overview#side-panel). This is done by [setting the activity starting state](https://developers.google.com/workspace/meet/add-ons/guides/collaborate-in-the-add-on) in JavaScript. All data that goes into the [`ActivityStartingState`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.activitystartingstate) should be set by the initiator of the add-on (typically the meeting host) within the side panel. You can think of the first view of the side panel as a form that controls the set up of your add-on.

### Close the side panel when not in use

After starting the activity by calling the [`startActivity()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetaddonclient.startactivity) method, you should only keep the [side panel](https://developers.google.com/workspace/meet/add-ons/guides/overview#side-panel) open if it's an essential part of the user experience for your Google Meet add-on. You can close the side panel once the [main stage](https://developers.google.com/workspace/meet/add-ons/guides/overview#main-stage) is open by calling the [`unloadSidePanel()`](https://developers.google.com/workspace/meet/add-ons/reference/websdk/addon_sdk.meetmainstageclient.unloadsidepanel) method.

### Promote your Meet add-on through screen sharing

Meet add-ons offer a richer experience than screen sharing. However, many users are accustomed to using Meet's screen sharing feature. If a user shares a tab displaying the website that hosts your Meet add-on, Meet can be configured to display a banner to all call participants prompting them to install or use the corresponding Meet add-on. For more information, see [promoting your add-on through screen sharing](https://developers.google.com/workspace/meet/add-ons/guides/promote#screen_sharing).
...
Do verify your dark-mode logo looks good in dark mode by using the [developer tools for Meet add-ons](https://developers.google.com/workspace/meet/add-ons/guides/devtools#setDarkMode).

Do verify your logo (and [other graphic assets](https://developers.google.com/workspace/marketplace/create-listing#graphic_assets)) look good in high contrast mode by using a contrast checker such as [Contrast Checker](http://webaim.org/resources/contrastchecker) by Web Accessibility In Mind (WebAIM).

Do adhere to the graphics [requirements for specific app integrations](https://developers.google.com/workspace/marketplace/about-app-review).

Don't include padding in your image. Extend the image to the boundaries of your file instead.

## Related topics

- [Troubleshoot and fix Meet add-on errors](https://developers.google.com/workspace/meet/add-ons/guides/troubleshoot)
