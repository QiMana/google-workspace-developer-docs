...
To read and write resources in import mode spaces, you must [authenticate Chat apps with a service account](./authenticate-authorize-chat-app.md). Grant the service account the following Chat API authorization scope by an administrator: `https://www.googleapis.com/auth/chat.import`.
...
No other [Google Chat API scopes](./authenticate-authorize.md#scopes) are required when importing resources into an import mode space. Chat apps can only import content into the import mode spaces that they create, and not into the import mode spaces created by other apps.
...
- To learn more about authentication and authorization, see [Authenticate and authorize Google Chat apps and API requests](./authenticate-authorize.md).
- To learn how to set up a service account to access the Google Chat API, see [Authenticate as a Google Chat app](./authenticate-authorize-chat-app.md).
- To learn how to start importing resources using import mode spaces, see [Import data to Google Chat](./import-data.md).
