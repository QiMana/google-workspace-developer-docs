...
With the Google Chat API, you can import data from your other messaging platforms into Google Chat. You can import existing messages, attachments, reactions, memberships, and space entities from your other messaging platforms to corresponding [Chat API resources](./api/reference/rest.md). You can import this data by creating Chat spaces in [*import mode*](./import-data-overview.md) and importing data into those spaces. After the process successfully completes, these spaces become standard Chat spaces.
...
- The Chat app must be delegated domain-wide authority in any domains that the app imports content in, see [Authorize Chat apps](./authorize-import.md).
...
Plan accordingly for the amount of data to be imported, understand how the [usage limits and quotas](./limits.md) can impact the import process, and be aware of the types of Chat spaces supported when importing to a new space. If you are an administrator, read [Import message data to Google Chat from another service](https://support.google.com/a/answer/13465849) and follow the steps carefully.

### Review API usage limits

The time required to import data into Chat can vary greatly depending on the quantity of Chat resources to import. Review your Chat app's [usage limits](./limits.md) and the amount of data scheduled for import from the source messaging platform to determine an estimated timeline.

When importing messages to a space, we recommend that you spread the calls to the [`messages.create()`](./api/reference/rest/v1/spaces.messages/create.md) method across different message threads.

### Identify the supported spaces to import

Import mode supports spaces with a [`SpaceType`](./api/reference/rest/v1/spaces.md#spacetype) of `SPACE` and `GROUP_CHAT`, but not `DIRECT_MESSAGE`.

To import conversations between two humans or a human and a Chat app, import the conversation to a two-member space with a [`SpaceType`](./api/reference/rest/v1/spaces.md#spacetype) of `SPACE` or `GROUP_CHAT`.

## Create a space in import mode

To create a space in import mode, call the [`create` method](./api/reference/rest/v1/spaces/create.md) on the [`Space` resource](./api/reference/rest/v1/spaces.md) and set `importMode` to `true`.
...
- **The date and time** - Remember, *import mode must be completed within 90 days.* If the space is still in import mode after 90 days from the time the `spaces.create()` method was called, it's automatically deleted and becomes inaccessible and unrecoverable.
	- Use the value of the [`importModeExpireTime`](./api/reference/rest/v1/spaces.md#Space) field to track the expiration of the 90 day timeframe.
		- Don't use the value of the [`createTime`](./api/reference/rest/v1/spaces.md#Space) field to track the expiration of the 90 day timeframe. This is not always the same as when you call the `spaces.create()` method. When using import mode, the `createTime` field can be set to the historical timestamp at which the space was created in the source in order to preserve the original creation time.
- **The space resource name ([`name`](./api/reference/rest/v1/spaces.md#Space.FIELDS.name))** - The unique identifier that is used to retrieve information about the specific space and is referenced in later steps when importing content into the space.
...
Your Chat apps can import messages using their own authority, or on behalf of a user through impersonation. The message author is set to the impersonated user account. For more information, see [Authorize Chat apps](./authorize-import.md). To import a message in an import mode space, call the [`create` method](./api/reference/rest/v1/spaces.messages/create.md) on the [`Message` resource](./api/reference/rest/v1/spaces.messages.md). In order to preserve the creation time of the original message from the source messaging platform, you can set the `createTime` of the message. This `createTime` must be set to a value between the space creation time that you previously set and present time.
...
When you create the messages in import mode, spaces don't notify or send email to any users, including messages which contain [user mentions](./format-messages.md#messages-@mention).
...
- `EMAIL`: the email address of the user account that you're impersonating with domain-wide authority.
- `SPACE_NAME`: the [name for the space created in import mode](./import-data.md#create-space).

### Reactions

Your Chat app can import [reactions for messages](./api/reference/rest/v1/spaces.messages.reactions.md) by using the Chat API. For information about the resource methods and types of authentication support in import mode spaces, see [Authorize Chat apps](./authorize-import.md).

### Attachments

Your Chat app can [upload attachments](./api/reference/rest/v1/media/upload.md) using the Chat API. For information about the resource methods and types of authentication support in import mode spaces, see [Authorize Chat apps](./authorize-import.md). However, we highly recommended that you use [Google Drive API](../drive/api/reference/rest/v3.md) to upload attachments as Google Drive files and link the file URIs to the respective messages in the import mode spaces to import attachments from other messaging platforms to avoid hitting Google Chat internal limit for attachment upload.

### Historical memberships

*Historical memberships* are memberships created for users who had already left the original space entity from the source messaging platform, but you want to [retain their data in Chat](https://support.google.com/vault/answer/7657594). For information about adding new members after the space is no longer in import mode, see [Create membership resource](./import-data.md#create_membership_resources).

In many cases, when those historical members are subject to a [data retention policy in Google](https://support.google.com/vault/answer/2462365), you want to preserve the data (such as Messages and reactions) created by historical memberships in a space before importing them to Chat. While the space is in import mode, you can import those historical memberships into the space, using the [`create` method](./api/reference/rest/v1/spaces.members/create.md) on the [`Membership` resource](./api/reference/rest/v1/spaces.members.md). In order to preserve the leave time of the historical membership, you must set the `deleteTime` of the membership. This leave time must be accurate because it impacts which data to retain for those memberships. Moreover, this `deleteTime` must be after the space creation timestamp and must not be a future timestamp.
...
- `EMAIL`: the email address of the user account that you're impersonating with domain-wide authority.
- `SPACE_NAME`: the [name for the space created in import mode](./import-data.md#create-space).
- `USER_ID`: the unique ID for the [user](../admin/directory/reference/rest/v1/users.md).
...
Your Chat app can read back and validate the contents of an import mode space by calling the [`list` method](./api/reference/rest/v1/spaces.messages/list.md) on the [`Message` resource](./api/reference/rest/v1/spaces.messages.md). You can read `Reaction` and `Attachment` resources from any returned message's `emojiReactionSummaries` and `attachment` fields. Chat apps can only call this method on behalf of a user through impersonation. For more information, see [Authorize Chat apps](./authorize-import.md).

Your Chat app can also read individual messages for validation by calling the [`get` method](./api/reference/rest/v1/spaces.messages/get.md) on the [`Message` resource](./api/reference/rest/v1/spaces.messages.md). Chat apps can only call this method to read their own messages by using their own authority. For more information, see [Authorize Chat apps](./authorize-import.md).

Chat apps can also list historical memberships by calling the [`list` method](./api/reference/rest/v1/spaces.members/list.md) on the [`Membership` resource](./api/reference/rest/v1/spaces.members.md). After the space exits import mode, the `list` method doesn't expose historical memberships anymore. Chat apps can only call this method on behalf of a user through impersonation. For more information, see [Authorize Chat apps](./authorize-import.md).

You can read an import mode space's properties by calling the [`get` method](./api/reference/rest/v1/spaces/get.md) on the [`Space` resource](./api/reference/rest/v1/spaces.md). The response will also have [`importModeExpireTime`](./api/reference/rest/v1/spaces.md#Space) populated, so you can properly track the timeframe to complete the import process. Chat apps can only call this method using their own authority. For more information, see [Authorize Chat apps](./authorize-import.md).
...
To update [supported fields](./api/reference/rest/v1/spaces.messages/patch.md#query-parameters) on a message in an import mode space, call the [`update` method](./api/reference/rest/v1/spaces.messages/patch.md) on the [`Message` resource](./api/reference/rest/v1/spaces.messages.md). Chat apps can only call this method using the same authority that was used during the initial message creation. If you used user impersonation during the initial message creation, you must use the same impersonated user to update that message.

To delete a message in an import mode space, call the [`delete` method](./api/reference/rest/v1/spaces.messages/delete.md) on the [`Message` resource](./api/reference/rest/v1/spaces.messages.md). Messages in an import mode space don't need to be deleted by the original message creator and can be deleted by impersonating any user in the domain. Chat apps can only delete their own messages using their own authority. For more information, see [Authorize Chat apps](./authorize-import.md).

### Reactions

To delete a reaction for a message in an import mode space, use the [`delete` method](./api/reference/rest/v1/spaces.messages.reactions/delete.md) on the `reactions` resource. For information about the resource methods and types of authentication support in import mode spaces, see [Authorize Chat apps](./authorize-import.md).

### Attachments

To update attachments for a message in an import mode space, use the [`upload` method](./api/reference/rest/v1/media/upload.md) on the `media` resource. For information about the resource methods and types of authentication support in import mode spaces, see [Authorize Chat apps](./authorize-import.md).

### Historical memberships

To delete a historical membership in an import mode space, use the [`delete` method](./api/reference/rest/v1/spaces.members/delete.md) on the [`Membership` resource](./api/reference/rest/v1/spaces.members.md). After a space exits import mode, the `delete` method doesn't let you delete historical memberships anymore.
...
To update [supported fields](./api/reference/rest/v1/spaces/patch.md#query-parameters) in an import mode space, use the [`patch` method](./api/reference/rest/v1/spaces/patch.md) on the `spaces` resource.

To delete an import mode space, use the [`delete` method](./api/reference/rest/v1/spaces/delete.md) on the `spaces` resource.

For information about the resource methods and types of authentication support in import mode spaces, and see [Authorize Chat apps](./authorize-import.md).

## Complete import mode

Before calling the [`completeImport`](./api/reference/rest/v1/spaces/completeImport.md) method, you should ensure that [validation](#validate-imported) and [reconciliation of resource differences](#reconcile-imported) are completed. Exiting a space out of import mode is an irreversible process and converts the import mode space into a regular space. There's no indicator in Chat that attributes these spaces to a data import.
...
To complete import mode and make the space accessible to users, the Chat app can call the `completeImport` method on the [`Space` resource](./api/reference/rest/v1/spaces.md). Chat apps can only call this method on behalf of a user through impersonation. For more information, see [Authorize Chat apps](./authorize-import.md). The impersonated user is added to the space as a [space manager](https://support.google.com/chat/answer/11833441) once this method completes. This method must be called within *90 days* of the initial `create.space` method call. If you attempt to call this method after the 90 day duration has elapsed, the call results in failures because the import mode space is deleted and is no longer accessible to the Chat app.
...
- **Add members to the space**: Call the [`create()` method](./api/reference/rest/v1/spaces.members/create.md) on the `Membership` resource. We recommend that Chat apps create `Membership` resources immediately after the space import is completed, so that Chat apps can continue to use the `chat.import` scope, and ensure that all imported members have access to the space. You should **prioritize** adding members that may be subject to the [Vault hold policy](https://support.google.com/vault/answer/7657594), which allows preservation of imported messages even if they are out-of-retention.
- **Set a target audience**: Call the [`update()` method](./api/reference/rest/v1/spaces/patch.md) on the `Space` resource. To learn how to create and add target audiences, see [Make a Google Chat space discoverable to specific users in a Google Workspace organization](./space-target-audience.md).

To use these methods with the `chat.import` scope, the impersonated user must be a space manager.

For external spaces, the membership [`create()` method](./api/reference/rest/v1/spaces.members/create.md) also allows to invite users outside of your Workspace organization. Make sure you understand all the [known limitations](https://support.google.com/a/answer/9296706) for external users.
...
If the space didn't import because it has too much data to import in the 90-day window with the current [usage limits](./limits.md), split the space into two or more smaller spaces for archiving purposes, and initiate the import process again.
