...
This guide explains how a Google Workspace administrator can use the [`search()` method](https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces/search) on a [`Space` resource](https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces) of the Google Chat API to manage named spaces across their organization and perform administrative tasks.

As a Google Workspace administrator, you can only use the Chat API to manage named spaces, which are defined as a [`SpaceType`](https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces#spacetype) of `SPACE`. Named spaces are persistent places where people send messages, share files, and collaborate. You can't use the Chat API to manage direct messages (DMs) or group chats.
...
- **Find and contact space managers.** Using the `search()` method, you can retrieve a list of all spaces in an organization, then use the [`members.list()` method](https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces.members/list) to retrieve a list of all space members that are managers.
...
- You must have **Manage Chat and Spaces conversation** [administrator privileges](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges) for your Google Workspace organization.

## Call the Google Chat API as an administrator

To authenticate as a user with [administrator privileges](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges), you must pass the following in your request:

- Specify an [authorization scope](https://developers.google.com/workspace/chat/authenticate-authorize#asynchronous-chat-calls) that supports using user authentication with administrator privileges.
...
To call the `search()` method with [user authentication](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user) and [administrator privileges](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges), you must specify the following in the request:

- Specify the `chat.admin.spaces` or `chat.admin.spaces.readonly` authorization scope.
- Specify `useAdminAccess=true`.
- Specify the search [`query`](https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces/search#body.QUERY_PARAMETERS.query) parameters to filter the results:
...
To search for a list of inactive spaces and then delete them with [user authentication](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user) and [administrator privileges](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges), pass the following in your requests:
...
- [Delete spaces](https://developers.google.com/workspace/chat/delete-spaces)
- [List members in a space](https://developers.google.com/workspace/chat/list-members)
- [Manage Chat spaces as a Google Workspace administrator](https://developers.google.com/workspace/chat/admin-overview)
